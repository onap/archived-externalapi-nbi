...
public class CheckOrderConsistenceManager {
    ...
    private boolean shouldAcceptServiceOrderItemToAdd(ServiceOrderItem serviceOrderItem,
        ...
        if (!StringUtils.isEmpty(serviceOrderItem.getService().getId())) {
            LOGGER
                .info("serviceOrderItem {0} for serviceorder {1} rejected cause service.id must be empty in add action",
                    serviceOrderItem.getId(), serviceOrderId);
            return false;
        } else if (!serviceOrderConsumerService.isTenantIdPresentInAAI()) {
            LOGGER.info("serviceOrderItem {0}  for serviceOrder {1} rejected cause tenantId not found in AAI",
                serviceOrderItem.getId(), serviceOrderId);
            return false;
        }
        return true;
    ...
    private boolean shouldAcceptServiceOrderItemToChange(ServiceOrderInfo serviceOrderInfo,
        ...
        if (StringUtils.isEmpty(serviceOrderItem.getService().getId())) {
            ...
            LOGGER
                .info("serviceOrderItem {0} for serviceOrder {1} rejected cause customer not found in inventory",
                    serviceOrderItem.getId(), serviceOrderInfo.getServiceOrderId());
            return false;
            ...
            LOGGER
                .info("serviceOrderItem {0} for serviceOrder {1} rejected cause service id {2} not found in inventory",
                    serviceOrderItem.getId(), serviceOrderInfo.getServiceOrderId(),
