...
public class CheckOrderConsistenceManager {
            ...
            switch (serviceOrderItem.getAction()) {
                ...
                case DELETE:
                    ...
                    if (isCustomerFromServiceOrderPresentInInventory(serviceOrderInfo)
                            && existServiceInInventory(serviceOrderItem,serviceOrderItemInfo,serviceOrderInfo.getSubscriberInfo().getGlobalSubscriberId())) {
                        serviceOrderInfo.addServiceOrderItemInfos(serviceOrderItem.getId(), serviceOrderItemInfo);
                    } else {
                        isServiceOrderRejected=true;
                        serviceOrderItem.setState(StateType.REJECTED);
    ...
    private boolean isCustomerFromServiceOrderPresentInInventory(ServiceOrderInfo serviceOrderInfo) {
        ...
        if (serviceOrderInfo.isUseServiceOrderCustomer()) {
            ...
            boolean customerPresentInAAI = serviceOrderConsumerService.isCustomerPresentInAAI(serviceOrderInfo.getSubscriberInfo().getGlobalSubscriberId());
            return customerPresentInAAI;
        }
        return true;
    }

    private boolean existServiceInInventory(ServiceOrderItem serviceOrderItem, ServiceOrderItemInfo serviceOrderItemInfo, String globalSubscriberId) {
        if (!StringUtils.isEmpty(serviceOrderItem.getService().getId())) {
            ...
            boolean serviceExistInInventory = serviceOrderConsumerService.doesServiceExistInServiceInventory(serviceOrderItem.getService().getId(), serviceName,globalSubscriberId );
    ...
    private void  handleServiceFromCatalog(ServiceOrderItem serviceOrderItem, ServiceOrderItemInfo serviceOrderItemInfo) {
        ResponseEntity<Object> response = serviceOrderConsumerService.getServiceCatalog(serviceOrderItem.getService().getServiceSpecification().getId());
        if (response!=null && ( response.getStatusCode().equals(HttpStatus.OK) || response.getStatusCode().equals(HttpStatus.PARTIAL_CONTENT)) ) {
