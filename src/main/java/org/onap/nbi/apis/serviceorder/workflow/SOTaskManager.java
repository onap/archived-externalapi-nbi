...
public class SOTaskManager {
     ...
     */
    private void registerOrderItemExecutionPlan(List<ServiceOrderItem> orderItems, ServiceOrderInfoJson serviceOrderInfoJson) {
        ...
        if(orderItems != null) {
            ...
            for (ExecutionTask executionTask : executionTasksSaved) {
                for (String key : internalIdOrderItemsMap.keySet()) {
                    String replace = executionTask.getReliedTasks().replace(key, String.valueOf(internalIdOrderItemsMap.get(key)));
