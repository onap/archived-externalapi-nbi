    ...
    private void registerOrderItemExecutionPlan(List<ServiceOrderItem> orderItems,
        ...
        if (orderItems != null) {
            ...
            for (ExecutionTask executionTask : executionTasksSaved) {
                List<String> orderIdList = new ArrayList<String>(Arrays.asList(executionTask.getReliedTasks().split(" ")));
                List<String> internalIDList = new ArrayList<String>();
                for (Entry<String, Long> entry : internalIdOrderItemsMap.entrySet()){
                   if(orderIdList.contains(entry.getKey())) {
                       internalIDList.add(entry.getValue().toString());
                    }
                }
            String reliedTasksString = String.join(" ", internalIDList);
